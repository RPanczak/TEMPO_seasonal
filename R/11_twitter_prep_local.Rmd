...
There are `r comma(sum(temp$freq))` tweets from `r comma(nrow(temp))` locations that have no geographic coordinates and `place_type` variable that indicates only country or large admin region (state). These tweets were removed from the data since little geographical analyses could be done here.

```{r}
tweets_clean_2019_07_22 %<>% 
  filter(!is.na(lat) & place_place_type != "country" & place_place_type != "admin")
...
[OpenCage](https://opencagedata.com) (via package `opencage`) was used to get coordinates for these places. If multiple places were found, then the first (probably best?) was kept.  
...
Note: only `tweets_geo2_inside` datset was prepared at this stage
