...
# twitter_places_geocoded - contains place_full_name for the top 39,999 occurring places, tweet count in the original tweet table, and the full geocode JSON result from Google's Maps API
...
```{r, eval=FALSE}
# Preprocessing done on HPC using script 11_twitter_prep_hpc.R
```
...
tweets_clean_2019_07_22 <- anti_join(tweets_clean_2019_07_22, select(geo2_outside_sp@data, lon, lat)) 
...
Large chunk of tweets - `r comma(sum(place_full_name$freq))` to be specific, have **no Twitter coordinates**. They are however labeled with `place_full_name`. Here are top 30 most frequent, with corresponding number of tweets missing coordinates from that places: 
...
Hovewer, this solution might also create bias towards more frequent places?!

[Google Maps Geocoding API](https://developers.google.com/maps/documentation/geocoding/intro) (via package `ggmap`) was used to get coordinates for these places.   

```{r eval=FALSE, include=FALSE}
frq(tweets_clean_2019_07_22$place_place_type) 
```
...
Here are some first, **experimental**, most frequent locations that are currently geocoded via Google. 

```{r}
place_full_name <- readRDS("./data/Twitter/clean/place_full_name_google.rds")

# place_full_name_sf <- place_full_name %>% 
#   drop_na(ext_lat) %>% 
#   filter(ext_lat != 0) %>% 
#   st_as_sf(coords = c("ext_lon", "ext_lat"), crs = 4326, remove = FALSE)
...
tweets_place <- tweets_clean_2019_07_22 %>% 
  filter(is.na(lat)) %>% 
...
sjmisc::frq(tweets_clean_2019_07_22, user_utc_offset, sort.frq = "desc", show.na = TRUE) 
sjmisc::frq(tweets_clean_2019_07_22, user_time_zone, sort.frq = "desc", show.na = TRUE) 
...
Note: only geolocated tweets (`tweets_geo2_inside` datset) have been prepared at this stage.
...
# tweets_clean_2019_07_22 <- readRDS(file = "./data/Twitter/clean/tweets_clean_2019_07_22.rds")
```
