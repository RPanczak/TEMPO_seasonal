...
pop_present_month <- pop_present %>% 
  ...
  group_by(start, end) %>% 
  summarize(daily_physically_present = mean(daily_physically_present))
...
```{r}
pop_present %>% 
  ggplot(aes(x=date, y=daily_physically_present)) +
  geom_segment(data = pop_present_month, 
               aes(x=start, xend=end, yend=daily_physically_present)) +
  geom_line(size = 1, colour = "darkorchid4") +
  scale_x_datetime(date_breaks = "2 months", date_minor_breaks = "1 month") + 
